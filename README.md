...
>
> **建议安装以下额外依赖：**
> - `playwright`：用于渲染公告、wiki图等功能。插件启动时会自动检测并安装 chromium 浏览器。如果自动安装失败，请手动执行 `uv run playwright install chromium`
> - `opencv-python`：用于面板图重复判断、提取面板图、相似度识别等功能
> - `fonttools`：用于多语言字体 fallback，未安装时日韩文可能显示为方框
>
> ```bash
> # Linux/Mac
> source .venv/bin/activate && uv pip install playwright opencv-python fonttools
> # Windows
> .venv\Scripts\activate; uv pip install playwright opencv-python fonttools
> ```
